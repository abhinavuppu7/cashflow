...
void _createDB(Database db,int version) async{
  await db.execute('CREATE TABLE $TABLE_TRANSACTIONS($COLUMN_ID INTEGER  PRIMARY KEY AUTOINCREMENT , $COLUMN_TRANSACTIONTYPE TEXT,$COLUMN_AMOUNT INTEGER)');
  ...
  final List<Map<String,dynamic>> result= await db.query( TABLE_TRANSACTIONS);
  return result;
  }

  Future<List<Transactions>>getTransactions ()async{
  final List<Map<String,dynamic>> transactionMapList= await getTransactionMapList();
...
{
  Database db= await this.db;
   final int result=await db.update(TABLE_TRANSACTIONS,
   transaction.toMap(),
   where: '$COLUMN_ID=?',
